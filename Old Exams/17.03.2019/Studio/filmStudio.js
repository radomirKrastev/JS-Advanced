    ...
    if (this.films.length) {
      for (let f of this.films) {
        ...
        if (roles.length) {
          ...
          let wantedRole = this.films[filmIndex].filmRoles.filter(
            (fR) => fR.role === role
          )[0];
  ...
  makeMovie(filmName, roles) {
    if (arguments.length === 2) {
      ...
      if (firstArgIsString && secondArgIsArray) {
        let findedFilms = this.films.filter((f) =>
          f.filmName.includes(filmName)
        );
...
// let studio = new FilmStudio("23");
// let movie = studio.makeMovie("Pirates", ["Az"]);
// let movie2 = studio.makeMovie("Pirates", ["Az"]);
